...
public class BuyPoint {
    ...
    private static final int openBuySellMenuId = Menus.registerMenu((player, option) -> {
    ...
    private static void openBuySellMenu(Player player) {
        ...
        Call.menu(player.con, openBuySellMenuId, "[red]Buy", "", buttons);
    }

    private static final int openSellMenuId = Menus.registerMenu((player, option) -> {
    ...
    private static void openSellMenu(Player player) {
        ...
        Call.menu(player.con, openSellMenuId, Bundle.get("menu.sellpoint.title", player.locale()), "", buttons);
    }

    private static final int quantityAdjustmentMenuForSellId = Menus.registerMenu((player, option) -> {
        Map<Item, Integer> quantities = getSelectedItemsQuantities(player);
        Item selectedItem = getSelectedItem(option);

        String[][] buttons = new String[][] { { "-2000", "-1000", "-100", "+400", "+1000", "+2000" },
                { "Sell", "Close", "Back" } };

        if (option < 6) { // Adjustment buttons
            int adjustment = Integer.parseInt(buttons[0][option]);
        ...
        } else if (option == 7) { // Close button
            player.sendMessage(Bundle.get("menu.sellpoint.close"));
            selectedItemsQuantities.remove(player);
        } else if (option == 8) { // Back button
            openSellMenu(player);
        }
    });
    ...
    private static void openQuantityAdjustmentMenuForSell(Player player, int option) {
        ...
        Call.menu(player.con(), quantityAdjustmentMenuForSellId, title, description, buttons);
    }

    private static final int openMenuId = Menus.registerMenu((player, option) -> {
    ...
    private static void openMenu(Player player) {
        ...
        Call.menu(player.con, openMenuId, Bundle.get("menu.buypoint.title", player.locale()), "", buttons);
    }
    ...
    private static Item getSelectedItem(int option) {
        Map<String, Object> selectedItemMap = Currency.items.get(option);
        return (Item) selectedItemMap.get("item");
    }
}